    ...
    <MainLayout>
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
            Overview of all your active and previous postings
          </p>
        </div>
        <button
          onClick={handlePostJob}
          className="bg-indigo-600 text-white px-5 py-2.5 sm:px-8 sm:py-3 rounded-xl sm:rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center justify-center self-end sm:self-auto gap-2 sm:gap-3 shadow-lg shadow-indigo-100/50 w-fit sm:w-auto"
        >
          <svg
            className="w-4 h-4"
            fill="none"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-8">
        {[
          ...
          {
            label: "Active",
            value: jobs.filter((j) => j.isActive && j.status === "open").length,
          ...
          <div
            key={i}
            className={`bg-white rounded-2xl shadow-sm border border-slate-100 p-4 sm:p-5 group hover:border-${stat.color}-100 transition-all duration-300`}
          >
            <div className="flex items-center justify-between mb-3">
              <div
                className={`p-2 bg-${stat.color}-50 rounded-xl group-hover:bg-${stat.color}-600 transition-colors`}
              >
                <svg
                  className={`w-5 h-5 text-${stat.color}-600 group-hover:text-white`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <span
                className={`text-[10px] font-black text-${stat.color}-600 uppercase tracking-widest bg-${stat.color}-50 px-2.5 py-1 rounded-full`}
              >
                {stat.label.split(" ")[0]}
              </span>
            </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span
                        className={`px-1.5 py-0.5 text-[9px] font-black uppercase rounded-lg border ${
                          job.isActive && job.status === "open"
                            ? "bg-emerald-50 text-emerald-700 border-emerald-100"
                            : "bg-rose-50 text-rose-700 border-rose-100"
                        }`}
